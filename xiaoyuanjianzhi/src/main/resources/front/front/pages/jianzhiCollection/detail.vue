	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jianzhiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jianzhiCollection', this.jianzhiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jianzhiCollectionPhoto ? this.detail.jianzhiCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jianzhiCollectionYuyue', {
