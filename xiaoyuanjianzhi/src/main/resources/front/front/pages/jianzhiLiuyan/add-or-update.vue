        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.jianzhiLiuyanText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`jianzhiLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.jianzhiId){
                this.ruleForm.jianzhiId = options.jianzhiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.jianzhiLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`jianzhiLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`jianzhiLiuyan`, this.ruleForm);
                }
