	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jianzhiLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jianzhiLiuyan', this.jianzhiLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jianzhiLiuyanPhoto ? this.detail.jianzhiLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jianzhiLiuyanYuyue', {
