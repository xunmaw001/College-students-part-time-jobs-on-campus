	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.jianzhiLiuyanText" type="text" placeholder="留言内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('jianzhiLiuyan','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
	<view style="">
        <view class="list b-b" v-for="(item, index) in list" :key="index"
            ...
            <view class="wrapper" style="padding: 10rpx 3rpx;margin-left: 25upx;margin-top: 10upx;">
                <view style="font-size: 30rpx;">
                    ...
                    <b>{{item.jianzhiName}}</b>
                </view>
                        <view class="address-box" style="padding: 10rpx;">
                            <text style="margin-left: 30rpx;">我的评价：{{item.jianzhiLiuyanText}}</text>
                        </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`jianzhiLiuyan`, params);
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`jianzhiLiuyan`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jianzhiLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`jianzhiLiuyan`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.jianzhiLiuyanText) {
                    searchForm['jianzhiLiuyanText'] = this.searchForm.jianzhiLiuyanText
                }

                let res = await this.$api.page(`jianzhiLiuyan`, searchForm);
				// 如果是第一页数据置空
