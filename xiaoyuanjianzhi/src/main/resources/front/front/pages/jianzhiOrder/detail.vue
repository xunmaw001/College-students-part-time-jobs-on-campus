	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.jianzhiOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jianzhiOrder', this.jianzhiOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jianzhiOrderPhoto ? this.detail.jianzhiOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jianzhiOrderYuyue', {
									yonghuId: _this.user.id,
									jianzhiOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jianzhiOrderYuyue/list');
							}
						}
					});
				},
