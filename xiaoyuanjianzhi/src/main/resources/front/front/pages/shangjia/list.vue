        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <view>
                        <uni-icons v-show="isAuth('shangjia','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.shangjiaPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">联系方式</text>
                     <text style="width: 70%">
                         <text v-if="item.shangjiaPhone != null">{{item.shangjiaPhone}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.shangjiaEmail" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">邮箱</text>
                     <text style="width: 70%">
                         <text v-if="item.shangjiaEmail != null">{{item.shangjiaEmail}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.shangjiaAddress" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">商家地址</text>
                     <text style="width: 70%">
                         <text v-if="item.shangjiaAddress != null">{{item.shangjiaAddress}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.shangjiaContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">商家介绍</text>
                    <view style="width: 70%" v-html="item.shangjiaContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('shangjia','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('shangjia','审核') && item.shangjiaYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`shangjia`, params);
			...
			async search() {
                ...
                let res = await this.$api.page(`shangjia`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.shangjiaContent!=null){
                        item.shangjiaContent = item.shangjiaContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
